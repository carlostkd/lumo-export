document.addEventListener('DOMContentLoaded', () => {
    ...
    const setStatus = (message, type = '', keep = false) => {
    ...
    // Proton Drive Feature
    const driveBtn = document.getElementById('driveBtn');

    function sendToDrive(content, filename, isBase64 = false) {
        return new Promise((resolve, reject) => {
            let timedOut = false;
            const timer = setTimeout(() => {
                timedOut = true;
                reject(new Error('Timed out waiting for bridge response (30s). Check ~/.lumo_drive_bridge.log'));
            }, 30000);

            let port;
            try {
                port = browser.runtime.connectNative("proton_drive_bridge");
            } catch (e) {
                clearTimeout(timer);
                reject(new Error('Bridge connection failed: ' + e.message));
                return;
            }
            ...
            port.onDisconnect.addListener(() => {
                ...
                if (err && err.includes("No such native application")) {
                    reject(new Error('Bridge not installed. Run install.sh first.'));
                } else {
                    reject(new Error(err || 'Bridge disconnected unexpectedly'));
                }
            });

            try {
                port.postMessage({ action: "upload", content, filename, parentPath: "/my-files/lumo-exports", isBase64 });
            } catch (e) {
                clearTimeout(timer);
    ...
    driveBtn.addEventListener('click', async () => {
        ...
        if (encryptCheck.checked) {
            setStatus('Encrypted exports are not supported for Drive upload yet.', 'error');
            return;
        }
        ...
        setStatus('Connecting to Proton Drive...', '');
        ...
        try {
            ...
            if (!response.success) {
                setStatus('❌ Upload failed: ' + (response.error || 'Unknown error'), 'error');
                return;
            }

            const imageResults = await browser.tabs.executeScript(tab.id, { code: SCAN_IMAGES_SCRIPT });
            const images = imageResults && imageResults[0] ? imageResults[0] : [];
            if (images.length === 0) {
                setStatus('✅ Uploaded to Proton Drive', 'success');
                return;
            }

            setStatus('Uploading ' + images.length + ' images... Don\'t close the popup', '', true);
            let uploaded = 0;
            for (let i = 0; i < images.length; i++) {
                ...
                if (imgResponse.success) {
                    ...
                    setStatus('⚠️ Image ' + (i+1) + ' failed: ' + (imgResponse.error || ''), 'error');
                }
            }

            if (uploaded === images.length) {
                setStatus('✅ Chat + ' + uploaded + ' images uploaded to Proton Drive', 'success');
            } else {
                setStatus('✅ Chat uploaded, ' + uploaded + '/' + images.length + ' images uploaded', '');
            }
...
`;
    ...
    const scanImages = async () => {
        ...
        try {
            const [tab] = await browser.tabs.query({ active: true, currentWindow: true });
            const result = await browser.tabs.executeScript(tab.id, { code: SCAN_IMAGES_SCRIPT });
            if (result && result[0]) {
